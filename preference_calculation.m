function [ynorm4,blue_avoidance] = preference_calculation(bin,skip,daylength,color_preference)
...
plot(ynorm4(1:end,2),'b');
hold on
plot(ynorm4(1:end,3),'r');
plot(ynorm4(1:end,1),'g');
errorbar(ynorm4(1:end,1),dayerr(:,1),'g')
errorbar(ynorm4(1:end,2),dayerr(:,2),'b')
errorbar(ynorm4(1:end,3),dayerr(:,3),'r')
title('Specify time for avergare blue avoidance');
% Choose time points to calculate average blue avoidance
[T,~]=ginput(2);                                                            
blue_avoidance(1) = mean(ynorm4(T(1):T(2),2));                              %average fraction of flies in blue (second color)
blue_avoidance(2) = mean(dayerr(T(1):T(2),2));                              %average deviation between hours of avoidance
blue_avoidance(3) = (mean(ynorm4(T(1):T(2),1))...                        %alternate blue avoidance index
    +mean(ynorm4(T(1):T(2),3))-mean(ynorm4(T(1):T(2),2))-1/3)*3/2;          % Index = (green+red-blue-1/3)*3/2 = 1-3*blue
